...
def get_merge_or_contribute_count_timeline(if_merged, ck_info):
    host = ck_info['host']
    port = ck_info['port']
    user = ck_info['user']
...
select search_key__owner, search_key__repo, author_email,month, map from (select search_key__owner, search_key__repo, author_email,month, area, sum(merge_counts) as merge_counts
from (select search_key__owner, search_key__repo, author_email, author_tz, '北美' as area,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      ...
      group by search_key__owner, search_key__repo, author_email, author_tz, month
      order by search_key__owner, search_key__repo, author_email, merge_counts desc


      union all

      select search_key__owner, search_key__repo, author_email, author_tz, '欧洲西部' as area,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '0时区' as area ,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '欧洲东部' as area ,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '印度' as area ,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '中国' as area ,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '日韩' as area,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
      ...
      select search_key__owner, search_key__repo, author_email, author_tz, '澳洲' as area ,toYYYYMM(authored_date) as month, count() as merge_counts
      from gits
      where (search_key__repo = 'rust' or
             search_key__repo = 'linux' or
             search_key__repo = 'llvm-project' or
...
group by search_key__owner, search_key__repo, author_email, area,month
order by search_key__owner,search_key__repo,author_email,month, merge_counts desc
limit 3 by search_key__owner,search_key__repo,author_email,month)

group by search_key__owner, search_key__repo, author_email,month
...
on a.search_key__owner = b.search_key__owner and a.search_key__repo = b.search_key__repo and a.author_email = b.commit__author__email) as a global left join
    ...
    filename = ''
    if if_merged == 1:
        filename = f"merge_timeline_{datetime.datetime.now().strftime('%Y-%m-%dT%H-%M-%SZ')}.csv"
    elif if_merged == 0:
        filename = f"contribute_timeline_{datetime.datetime.now().strftime('%Y-%m-%dT%H-%M-%SZ')}.csv"
    with open(filename, "w", encoding='utf8',
        ...
        for result in results:
            ...
            month = result[3]
            author_tz = result[4]
            total_merge_count = result[5]
            github_id = result[6]
            company = result[7]
            emain_domain = result[8]
            # print(company)
...
if __name__ == '__main__':
    ...
    if_merged = 1
    get_merge_or_contribute_count_timeline(if_merged, ck_info)
    if_merged = 0
    time.sleep(2)
    get_merge_or_contribute_count_timeline(if_merged, ck_info)
